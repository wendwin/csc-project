<div x-show="sidebarOpen" x-transition.opacity @click="sidebarOpen = false"
    class="fixed inset-0 z-60  bg-black bg-opacity-50 md:hidden"></div>
...
<aside x-show="sidebarOpen" x-transition @click.away="sidebarOpen = false"
    ...
    <form method="POST" action="{{ route('admin.logout') }}" class="absolute bottom-6 w-full px-4">
        @csrf
        <button type="submit"
            class="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-500 hover:text-red-500">
...
{{-- SIDEBAR DESKTOP --}}
<aside
    class="bg-[#D9D9D9] shadow-xl h-screen  overflow-y-auto fixed top-0 left-0 z-40 hidden md:block transition-all duration-300"
    :class="sidebarOpen ? 'w-64' : 'w-20'">
    <div class="px-6 py-4 text-2xl flex justify-center items-center">
        <div class="transition-all duration-200 flex items-center mr-11" x-show="sidebarOpen" x-cloak>
            <h3 class="text-blue-700 font-bold text-2xl">Dash<span class="text-black">Publikasi</span></h3>
        </div>
        <div class="transition-all duration-200 flex items-center space-x-0" x-show="!sidebarOpen" x-cloak>
    ...
    {{-- Menu Utama --}}
    <nav class="px-4 mt-2 space-y-2 text-gray-700 font-medium">
