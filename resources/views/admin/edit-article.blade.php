...
<form action="{{ route('articles.update', $article->id) }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        {{-- Judul --}}
        <div>
            <label class="block mb-1 text-sm font-semibold text-gray-700">Judul Artikel</label>
            <input type="text" name="title" required value="{{ old('title', $article->title) }}"
                class="w-full px-3 py-2 border border-blue-300 rounded-lg shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
        ...
        <div class="mb-4">
            ...
            <select id="target_website" name="target_website"
                class="w-full px-3 py-2 border border-blue-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                <option value="">-- Pilih Target Website --</option>
                <option value="csc" {{ old('target_website', $article->target_website ?? '') === 'csc' ? 'selected' : ''
                    }}>CSC
                </option>
                <option value="pustaka-pemda" {{ old('target_website', $article->target_website ?? '') ===
                    'pustaka-pemda' ? 'selected' : '' }}>Pustaka Pemda</option>
                <option value="pspi" {{ old('target_website', $article->target_website ?? '') === 'pspi' ? 'selected' :
                    '' }}>PSPI
                </option>
            </select>
        </div>
        ...
        {{-- Gambar Utama --}}
        <div>
            <label class="block mb-1 text-sm font-semibold text-gray-700">Gambar Utama</label>
            <input type="file" name="main_image" accept="image/*"
                class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">

            @if (!empty($article->main_image))
            <div class="mt-3">
                <img src="{{ asset('storage/' . $article->main_image) }}" alt="Preview Gambar Utama"
                    class="object-cover w-32 h-24 border rounded-lg shadow">
            </div>
            @endif
        </div>
        ...
        <div>
            <label class="block mb-1 text-sm font-semibold text-gray-700">Upload Gambar Tambahan Baru</label>
            <input type="file" name="images[]" multiple accept="image/*"
                class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-sm file:bg-green-50 file:text-green-700 hover:file:bg-green-100">
            @error('images.*')
            ...
            <div class="grid grid-cols-2 gap-4 mt-3 md:grid-cols-4">
                ...
                <div class="w-full h-24 overflow-hidden border rounded-lg shadow">
    ...
    {{-- Konten --}}
    <div>
        <label class="block mb-2 text-sm font-semibold text-gray-700">Konten Artikel</label>
    ...
    <div class="flex items-center justify-between pt-4">
        <a href="{{ route('articles.index') }}"
            class="bg-[#FF0000] hover:bg-red-600 text-white font-semibold px-4 py-2 rounded-lg shadow text-[16px] transition duration-200 cursor-pointer">Kembali</a>
        <button type="submit"
